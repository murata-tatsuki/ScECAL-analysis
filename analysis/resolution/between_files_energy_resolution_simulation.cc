...
{ 
  ...
  for (int i = 0; i < Nenegry; ++i) {
    // if(energy[i]==0.5) energy_deposition[i] = new TH1F(Form("energy_deposition_%d",i),Form("%d GeV energy deposition",(int)energy[i]),500,0,100);
    energy_deposition[i] = new TH1F(Form("energy_deposition_%d",i),Form("%d GeV energy deposition",(int)energy[i]),range_maximum,0,range_maximum);
    // energy_deposition[i] = new TH1F(Form("energy_deposition_%d",i),Form("%d GeV energy deposition",(int)energy[i]),range_maximum,0,range_maximum);
    ...
    energy_deposition[i]->SetXTitle("Detected Energy [MeV]");
    
    energy_deposition_bestChannels[i] = new TH1F(Form("energy_deposition_bestChannels_%d",i),Form("%d GeV energy deposition",(int)energy[i]),range_maximum,0,range_maximum);
  ...
  TF1 *gaussian = new TF1("gaussian","gaus",0,range_maximum);
  // gaussian->SetNpx(range_maximum);
  // TF1 *gaussian_PS = new TF1("gaussian_PS","gaus",2,range_maximum_PS);
  ...
  for(int ienegry=0; ienegry<Nenegry; ienegry++){
    C_res[ienegry] = new TCanvas(Form("Edep_%dGeV",(int)energy[ienegry]), Form("%d GeV energy deposit",(int)energy[ienegry]), 1);
    C_res[ienegry]->cd();

    // energy_deposition[ienegry]->GetXaxis()->SetRange(2,range_maximum_PS);
    int histo_range_minimum = energy_deposition[ienegry]->GetMean() - energy_deposition[ienegry]->GetRMS() * 4;
    int histo_range_maximum = energy_deposition[ienegry]->GetMean() + energy_deposition[ienegry]->GetRMS() * 4;
    ...
    double rms = energy_deposition[ienegry]->GetRMS();
    // if(energy[ienegry]<=5){
    //   amp = 500;
    //   mean = energy[ienegry] * 40;
    // }
    if(energy[ienegry]==0.5) cout << histo_range_minimum << "  " << histo_range_maximum  << "  " << amp << "  " << mean << "  " << endl;
    
    gaussian->SetParameter(0,amp);
    gaussian->SetParameter(1,mean);
    // energy_deposition[ienegry]->Fit("gaussian","","NQ",mean-50*(energy[ienegry]/20),mean+50*(energy[ienegry]/20));
    energy_deposition[ienegry]->Fit("gaussian","","NQ",mean-rms*4,mean+rms*4);
    // if(energy[ienegry]>5) energy_deposition[ienegry]->Fit("gaussian","","NQ",mean-50*(energy[ienegry]/20),mean+50*(energy[ienegry]/20));
    // else energy_deposition[ienegry]->Fit("gaussian","","NQ",40*energy[ienegry]-10*energy[ienegry],40*energy[ienegry]+10*energy[ienegry]);
    ...
    C_res_bestChannels[ienegry] = new TCanvas(Form("Edep_%dGeV_bestChannels",(int)energy[ienegry]), Form("%d GeV energy deposit",(int)energy[ienegry]), 1);
    C_res_bestChannels[ienegry]->cd();
    histo_range_minimum = energy_deposition_bestChannels[ienegry]->GetMean() - energy_deposition_bestChannels[ienegry]->GetRMS() * 4;
    histo_range_maximum = energy_deposition_bestChannels[ienegry]->GetMean() + energy_deposition_bestChannels[ienegry]->GetRMS() * 4;
    energy_deposition_bestChannels[ienegry]->GetXaxis()->SetRange(histo_range_minimum,histo_range_maximum);
    gaussian->SetNpx(range_maximum);
    // if(energy[ienegry]<=5) energy_deposition[ienegry]->GetXaxis()->SetRange(10/binWidth_PS,range_maximum_PS/binWidth_PS);
    amp = energy_deposition_bestChannels[ienegry]->GetMaximum();
    mean = energy_deposition_bestChannels[ienegry]->GetMaximumBin();
    rms = energy_deposition_bestChannels[ienegry]->GetRMS();
    if(energy[ienegry]==0.5) cout << histo_range_minimum << "  " << histo_range_maximum  << "  " << amp << "  " << mean << "  " << endl;
    gaussian->SetParameter(0,amp);
    gaussian->SetParameter(1,mean);
    energy_deposition_bestChannels[ienegry]->Fit("gaussian","","NQ",mean-rms*4,mean+rms*4);
  ...
  Eres->SetMinimum(0);
  Elinear->SetTitle("Energy Linearity");
  Elinear->GetXaxis()->SetTitle("Beam Energy [GeV]");
  Elinear->GetYaxis()->SetTitle("Reconstructed Energy [MeV]");

  TGraphErrors *Eres_lowEnergy = new TGraphErrors();
  Eres_lowEnergy->SetTitle("Energy Resolution");
  Eres_lowEnergy->SetMinimum(0);
  Eres_lowEnergy->GetXaxis()->SetTitle("Beam Energy [GeV]");
  Eres_lowEnergy->GetYaxis()->SetTitle("Energy Resolution sigma(E)/E");
  ...
  for(int i=0;i<3;i++){
    energy_distributions[i] = new TCanvas(Form("energy_distributions_%d",i), Form("energy_distributions_%d",i), 1); 
  ...
  Elinear_bestChannels->SetTitle("Energy Linearity");
  Elinear_bestChannels->GetXaxis()->SetTitle("Beam Energy [GeV]");
  Elinear_bestChannels->GetYaxis()->SetTitle("Reconstructed Energy [MeV]");

  TGraphErrors *Eres_lowEnergy_bestChannels = new TGraphErrors();
  Eres_lowEnergy_bestChannels->SetTitle("Energy Resolution");
  ...
  Eres_lowEnergy_bestChannels->Fit("res_func_low");
  ...
  Eres_lowEnergy_bestChannels->Write("Eres_lowEnergy_bestChannels");
  ...
  for(int i=0;i<3;i++){
    energy_distributions_bestChannels[i] = new TCanvas(Form("energy_distributions_bestChannels_%d",i), Form("energy_distributions_bestChannels_%d",i), 1); 
