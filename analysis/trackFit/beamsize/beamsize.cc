...
// SSAを使ってはじめのlayerのhit mapを求めるコード
// track fitを使ってはじめのlayerでのbeamを求めるコード
...
{ 
  ...
  // vector<string> skipFiles{"ECAL_Run196_20230504_123821.root", "ECAL_Run197_20230504_135137.root", "ECAL_Run194_20230504_082851.root", "ECAL_Run195_20230504_104136.root", "AHCAL_Run133_20230501_081117.root"};
    ...
    for (int i_layer = 0; i_layer < layerNu; ++i_layer) {
      for (int i_chip = 0; i_chip < chipNu; ++i_chip) {
        for (int i_channel = 0; i_channel < channelNu; ++i_channel) {
          edep_channel[i_layer][i_chip][i_channel] = new TH1F(Form("edep_channel_%d_%d_%d",i_layer, i_chip, i_channel),Form("%g GeV layer%d chip%d channel%d",energy[ienegry], i_layer, i_chip, i_channel),2000,0,2);
        }
      }
    }
    ...
    for(int irawfile=0; irawfile<energy_files[ienegry]; irawfile++){
      ...
      // if (result != skipFiles.end()) continue;
        ...
        for(int ihit=0;ihit<Hit_Energy->size();ihit++){
          ...
          edep_1hit[ienegry][SiPMType]->Fill(Hit_Energy->at(ihit));
        }
    ...
    fileout.cd();
    fileout.mkdir(Form("%dGeV", (int)energy[ienegry]));
    fileout.cd(Form("%dGeV", (int)energy[ienegry]));
    for (int i_layer = 0; i_layer < 30; ++i_layer) {
      for (int i_chip = 0; i_chip < chipNu; ++i_chip) {
        ...
        c1->Update();
        c1->Write(Form("edep_Layer%d_Chip%d", i_layer, i_chip));
        c1->SaveAs(Form("%s/%gGeV/%gGeV_layer%dchip%d.png",argv[argc-1],energy[ienegry],energy[ienegry],i_layer,i_chip));
        delete c1;
        for (int i_channel = 0; i_channel < channelNu; ++i_channel) {
