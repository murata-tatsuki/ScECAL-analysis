...
{ 
  ...
  ifstream filelist(Form("./datalist_%s.txt",beamType.c_str()));
  ...
  const string savePath = Form("../../result/trackFit/efficiency/%s/%s.root",beamType.c_str(),runName.c_str());
  TFile fileout(Form("",savePath.c_str()),"RECREATE");
