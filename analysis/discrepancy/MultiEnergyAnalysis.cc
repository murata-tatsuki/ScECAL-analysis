...
nameStrings find_name_of_file(string filePath){
  ...
  if(result.dataType == "simulation"){
    ...
    result.threshold = path;
  }

  filePath.erase(0,filePath.find_last_of("_")+1);
  result.cog_range = stoi(filePath);
  ...
  cutIds[10]  = 2;
  cutIds[20]  = 3;
  map<string, int> threIds;
  threIds["threshold"]   = 0;
  threIds["threshold0"]  = 1;
  threIds["threshold50"] = 2;

  result.isData   = (result.dataType == "data");
  result.cutIdx   = cutIds[result.cog_range];
  result.calibIdx = threIds[result.threshold];
...
TLegend* CreateNiceLegend(double x1, double y1, double x2, double y2, int nCols = 2) {
  ...
  leg->SetNColumns(nCols);             // 列数の指定（2〜4列が見やすい）
  leg->SetBorderSize(0);               // 黒い枠線を消す
  
  // 【背景設定】グラフの線と被らないように「80%透過の白」にする（完全透明なら SetFillStyle(0)）
  leg->SetFillColorAlpha(kWhite, 0.8); 
  
  leg->SetTextFont(42);                // Helvetica（論文標準の綺麗なノンセリフ体）
  leg->SetTextSize(0.03);              // 文字サイズを適切に指定
...
int CalculateRebinFactor(const TH1* h, double sigma) {
    ...
    // 1. ヒストグラムから現在の 1 ビンの幅を取得 (第1ビンの幅)
    double currentBinWidth = h->GetBinWidth(1);
    if (currentBinWidth <= 0.0) return 1;

    // 2. 目標とするビン幅を計算 (例: sigma / 5.0)
    double targetBinWidth = sigma / 10.0;

    // 3. Rebin Factor (何個のビンを1つにまとめるか) を計算して四捨五入
    int rebinFactor = static_cast<int>(std::floor(targetBinWidth / currentBinWidth));

    // 4. 最低でも 1 (Rebin しない) を保証する
    return std::max(1, rebinFactor);
}

int CalculateLandauRebinFactor(const TH1* h, int targetBinsOnRise = 10) {
    if (!h) return 1;
    ...
    // ピークより左側に向かって Half Maximum となるビンを探す
    int halfMaxBin = maxBin;
    for (int b = maxBin; b >= 1; --b) {
        if (h->GetBinContent(b) <= halfMax) {
            halfMaxBin = b;
            break;
        }
    }
    ...
    // 立ち上がり部分の現在のビン数
    int currentBinsOnRise = maxBin - halfMaxBin;
    if (currentBinsOnRise <= 0) return 1;

    // 目標のビン数（例: 8ビン）になるように Rebin Factor を計算
    int rebinFactor = currentBinsOnRise / targetBinsOnRise;
...
{ 
  ...
  // int irawfilenum = 3 + 2*rawfilenum;
  ...
  fileout.mkdir("data");
	...
	// dataを入れるもの
  TH1F* energy_deposition[rawfilenum][Ndatasim];
  TF1 *fit_gaus[rawfilenum][Ndatasim];
  TH2F* hitmap[rawfilenum][layerNu][Ndatasim];
  TH1F* number_of_hits[rawfilenum][Ndatasim];
  TF1 *fit_gaus_nhit[rawfilenum][Ndatasim];
  TH2F* hit_vs_e[rawfilenum][Ndatasim];
  TH2F* layer_vs_edep[rawfilenum][Ndatasim];

  TH1F* energy_deposition_bestChannels[rawfilenum][Ndatasim];
  TH2F* hitmap_bestChannels[rawfilenum][layerNu][Ndatasim];
  TH1F* number_of_hits_bestChannels[rawfilenum][Ndatasim];
  TH2F* hit_vs_e_bestChannels[rawfilenum][Ndatasim];
  TH2F* layer_vs_edep_bestChannels[rawfilenum][Ndatasim];

  TH1F* hit_per_layer[rawfilenum][layerNu][Ndatasim];
  TH1F* hit_per_layer_bestChannels[rawfilenum][layerNu][Ndatasim];
  TH1F* edep_per_layer[rawfilenum][layerNu][Ndatasim];
  TH1F* edep_per_layer_bestChannels[rawfilenum][layerNu][Ndatasim];

  TH2F* cog[rawfilenum][layerNu][Ndatasim];
  TH1F* shower_radiusRMS[rawfilenum][Ndatasim];
  TH1F* shower_radius90[rawfilenum][Ndatasim];
  TH2F* layer_vs_rRMS[rawfilenum][Ndatasim];
  TH2F* layer_vs_r90[rawfilenum][Ndatasim];
  TH1F* edep_1hit[rawfilenum][2][Ndatasim];
  TH1F* edep_1hit_allRagne[rawfilenum][2][Ndatasim];
  // TH2F* edep_vs_rRMS[rawfilenum];
  // TH2F* edep_vs_r90[rawfilenum];

  
  
	// 保存形式
  TGraphErrors *Eres[Ndatasim];
  for(int ds=0;ds<Ndatasim;ds++){
    Eres[ds] = new TGraphErrors();
    Eres[ds]->SetTitle("Energy Resolution");
    // Eres[ds]->SetTitle("Energy Resolution (all channels)");
    Eres[ds]->GetXaxis()->SetTitle("Beam Energy [GeV]");
    Eres[ds]->GetYaxis()->SetTitle("Detected Energy Resolution sigma(E)/E");
  }
  ...
  string dirNames[2] = {"beforeEventCut", "afterEventCut"};
  // string DataSim[2] = {"data", "simulation"};
  string histoDrawOptions[Ndatasim];
  string graphDrawOptions[Ndatasim];
  // Color_t histo_colors[2] = { kBlack  , kAzure - 9 };
  // Color_t fit_colors[2]   = { kRed + 1, kBlue + 1  };
  for(int ds=0;ds<Ndatasim;ds++){
    if(ds==0){
      histoDrawOptions[ds] = "HIST";
      graphDrawOptions[ds] = "AP";
    } else {
      histoDrawOptions[ds] = "HIST same";
      graphDrawOptions[ds] = "P";
    }
  }
  ...
  for(int ienergy=0; ienergy<rawfilenum; ienergy++){
    if(filenames[0][ienergy].first != filenames[1][ienergy].first){
      cout << "wrong in energy sort !!" << endl;
      return 1;
    }
    double _energy = filenames[0][ienergy].first;
    // if(rawfilenum>1) cout << _energy << " GeV" << endl;
    fileout.cd();
    fileout.mkdir(Form("%dGeV", (int)_energy));
    fileout.cd(Form("%dGeV", (int)_energy));
    ...
    for(int ds=0;ds<Ndatasim;ds++){
      ...
      double mean = fit_gaus[ienergy][ds]->GetParameter(1);
      double mean_error = fit_gaus[ienergy][ds]->GetParError(1);
      double sigma = fit_gaus[ienergy][ds]->GetParameter(2);
      double sigma_error = fit_gaus[ienergy][ds]->GetParError(2);
      ...
      if(ds==0) number_of_hits[ienergy][ds]->GetXaxis()->SetRangeUser(fit_gaus_nhit[ienergy][ds]->GetParameter(1)-10*fit_gaus_nhit[ienergy][ds]->GetParameter(2),fit_gaus_nhit[ienergy][ds]->GetParameter(1)+10*fit_gaus_nhit[ienergy][ds]->GetParameter(2));
      number_of_hits[ienergy][ds]->Draw(histoDrawOptions[ds].c_str());
      ...
      edep_1hit[ienergy][0][ds]->GetXaxis()->SetRangeUser(300,2000);
      edep_1hit[ienergy][0][ds]->GetYaxis()->SetTitle("[a.u.]");
      edep_1hit[ienergy][0][ds]->Scale(1./edep_1hit[ienergy][0][ds]->Integral());
      edep_1hit[ienergy][0][ds]->Draw(histoDrawOptions[ds].c_str());
      C_e_summary->cd(4);
      ...
      edep_1hit[ienergy][1][ds]->GetXaxis()->SetRangeUser(300,2000);
      edep_1hit[ienergy][1][ds]->GetYaxis()->SetTitle("[a.u.]");
      edep_1hit[ienergy][1][ds]->Scale(1./edep_1hit[ienergy][1][ds]->Integral());
      edep_1hit[ienergy][1][ds]->Draw(histoDrawOptions[ds].c_str());
      ...
      edep_1hit_allRagne[ienergy][0][ds]->Scale(1./edep_1hit_allRagne[ienergy][0][ds]->Integral());
      edep_1hit_allRagne[ienergy][0][ds]->Draw(histoDrawOptions[ds].c_str());
      C_e_summary->cd(8);
      ...
      edep_1hit_allRagne[ienergy][1][ds]->Scale(1./edep_1hit_allRagne[ienergy][1][ds]->Integral());
      edep_1hit_allRagne[ienergy][1][ds]->Draw(histoDrawOptions[ds].c_str());
      ...
      for (int i_layer = 0; i_layer < layerNu; ++i_layer) {
        ...
        gPad->SetLogy(1);
        edep_per_layer[ienergy][i_layer][ds]->GetXaxis()->SetRangeUser(300,2000);
        edep_per_layer[ienergy][i_layer][ds]->Draw(histoDrawOptions[ds].c_str());
      }
    ...
    C_e_summary->cd();
    C_e_summary->cd(1);
    energy_deposition[ienergy][0]->SetMaximum(max_height_edep * 1.15);
    legHisto->Draw("same");
    gPad->Modified();
    gPad->Update();
    C_e_summary->cd(2);
    ...
    gPad->Update();
    C_e_summary->Write();
    C_e_summary->SaveAs(Form("%s/summary_%gGeV.png",argv[argc-1],_energy));

    C_edeps->Write();
    C_edeps->SaveAs(Form("%s/%gGeV_edeps.png",argv[argc-1],_energy));
  ...
  auto FitFuncResolution = [&enable_reject](double *x, double *p) {
    ...
    if (enable_reject && ( (x[0] > 25.0 && x[0] < 35.0) || (x[0] > 75.0 && x[0] < 85.0) )) {
        TF1::RejectPoint();
  ...
  for(int ds=0;ds<Ndatasim;ds++){
    ...
    enable_reject = true;
    Eres[ds]->Fit(Form("res_func_low_%d",ds), "","", 0.1,130);
  ...
  gStyle->SetOptFit(0);
  resolution_plots->SetGrid();
  TLegend *legResData = CreateNiceLegend(0.3, 0.4, 0.9, 0.85, 1);
  for(int ds=0;ds<Ndatasim;ds++){
  ...
  gStyle->SetOptFit(0);
  resolution_plots->SetGrid();
  TLegend *legResSim = CreateNiceLegend(0.3, 0.4, 0.9, 0.85, 1);
  int firstDrawing = 0;
  for(int ds=0;ds<Ndatasim;ds++){
    if(drawSettings[ds].isData) continue;
    if(firstDrawing==0) Eres[ds]->Draw("AP");
