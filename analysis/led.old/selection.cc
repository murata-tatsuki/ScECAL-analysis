...
{ 
  if(argc < 3 && argc%2==1){                                                     //エラー処理
    cout << "usage: ./selection output.root input1_ssa.root input1_decode.root  ..." << endl;
    return 1;
  }
  ...
  */
  ...
  fileout.mkdir("chip_canvas_ssaCut");
  // gain_histo->Write();
  ...
  for (int i_layer = 0; i_layer < layerNu; ++i_layer) {
    for (int i_chip = 0; i_chip < chipNu; ++i_chip) {
      for (int i_channel = 0; i_channel < channelNu; ++i_channel) {
        int bin = ( 3 < i_layer && i_layer < 28 ) ? bin_10 : bin_15;
        int min = ( 3 < i_layer && i_layer < 28 ) ? min_10 : min_15;
        int max = ( 3 < i_layer && i_layer < 28 ) ? max_10 : max_15;
        sprintf(char_tmp, "Layer%d_Chip%d_Chn%d", i_layer, i_chip, i_channel);
        h_MIP[i_layer][i_chip][i_channel] = new TH1D(char_tmp, char_tmp, bin, min, max);
          // h_MIP[i_layer][i_chip][i_channel] = new TH1D(char_tmp, char_tmp, 150, -50, 1000);
          // h_MIP[i_layer][i_chip][i_channel] = new TH1D(char_tmp, char_tmp, 210, -50, 1000);
          // h_MIP[i_layer][i_chip][i_channel] = new TH1D(char_tmp, char_tmp, 150, -100, 3000);
          // h_MIP[i_layer][i_chip][i_channel] = new TH1D(char_tmp, char_tmp, 620, -100, 3000);
        ...
        sprintf(char_tmp, "ssaCut_Layer%d_Chip%d_Chn%d", i_layer, i_chip, i_channel);
        h_MIP_ssaCut[i_layer][i_chip][i_channel] = new TH1D(char_tmp, char_tmp, bin, min, max);
          // h_MIP_ssaCut[i_layer][i_chip][i_channel] = new TH1D(char_tmp, char_tmp, 150, -50, 1000);
          // h_MIP_ssaCut[i_layer][i_chip][i_channel] = new TH1D(char_tmp, char_tmp, 150, -100, 3000);
        h_MIP_ssaCut[i_layer][i_chip][i_channel]->GetXaxis()->SetTitle("ADC");
        h_MIP_ssaCut[i_layer][i_chip][i_channel]->SetLineWidth(2);
        h_MIP_ssaCut[i_layer][i_chip][i_channel]->SetLineColor(4);

        sprintf(char_tmp, "Layer%d_Chip%d_Chn%d_tempCorrected", i_layer, i_chip, i_channel);
        h_MIP_tempCorrected[i_layer][i_chip][i_channel] = new TH1D(char_tmp, char_tmp, bin, min, max);
          // h_MIP_tempCorrected[i_layer][i_chip][i_channel] = new TH1D(char_tmp, char_tmp, 1050, -50, 1000);
          // h_MIP_tempCorrected[i_layer][i_chip][i_channel] = new TH1D(char_tmp, char_tmp, 3100, -100, 3000);
  ...
  for(int irawfile=0; irawfile<rawfilenum; irawfile++){
    ...
    // tree[irawfile]->SetBranchAddress("GainTag", &gainTags_ssa);
  ...
  fileout.cd();
  fileout.cd("chip_canvas_ssaCut");
  cout << "chip_canvas_ssaCut" << endl;
  for (int i_layer = 0; i_layer < layerNu; ++i_layer) {
    cout << i_layer << "/" << layerNu << endl;
    for (int i_chip = 0; i_chip < chipNu; ++i_chip) {
      for (int i_channel = 0; i_channel < channelNu; ++i_channel) {
        h_MIP_ssaCut[i_layer][i_chip][i_channel]->Write();
        // h_MIP[i_layer][i_chip][i_channel]->Delete();
      }
    }
  }
  ...
  TCanvas* C_FFT_ssaCut[layerNu][chipNu];
  ...
  for (int i_layer = 0; i_layer < layerNu; ++i_layer) {
    ...
    for (int i_chip = 0; i_chip < chipNu; ++i_chip) {
      ...
      sprintf(char_tmp, "layer%d_chip%d_ssaCut_freq", i_layer, i_chip);
      C_FFT_ssaCut[i_layer][i_chip] = new TCanvas(char_tmp, char_tmp, 1);
      C_FFT_ssaCut[i_layer][i_chip]->Divide(6, 6);
      ...
      for (int i_channel = 0; i_channel < channelNu; ++i_channel) {
        ...
        double mean=0, rms=0;
        const int nb = h_MIP_ssaCut[i_layer][i_chip][i_channel]->GetNbinsX();
        const double xmin_ = h_MIP_ssaCut[i_layer][i_chip][i_channel]->GetXaxis()->GetXmin() - mean;
        const double xmax_ = h_MIP_ssaCut[i_layer][i_chip][i_channel]->GetXaxis()->GetXmax() - mean;

        TH1D* hCorr = new TH1D("tmp_hCorr","ped sub;ADC-ped (ADC);counts", nb, xmin_, xmax_);
        hCorr->SetDirectory(nullptr);
        for(int b=1;b<=nb;++b){
          double xraw=h_MIP_ssaCut[i_layer][i_chip][i_channel]->GetBinCenter(b);
          double y   =h_MIP_ssaCut[i_layer][i_chip][i_channel]->GetBinContent(b);
        ...
        auto pk=pickK0(hFFT,Npad,i_layer);
        double phase = estimatePhase(hCorr,pk.gain);
        TF1* f=makeCos(hCorr,pk.gain,phase,"fWave");
        double xmin=hCorr->GetXaxis()->GetXmin(), xmax=hCorr->GetXaxis()->GetXmax();
        double ymin=f->GetMinimum(xmin,xmax), ymax=std::max(hCorr->GetMaximum(), f->GetMaximum(xmin,xmax));
        hCorr->SetMinimum(std::min(0.0,ymin*1.1));
        hCorr->SetMaximum(ymax*1.1);
        if(i_layer<4 || i_layer>27) hCorr->GetXaxis()->SetRange(((int)250/binw_15),((int)600/binw_15));
        if(i_layer>=4 && i_layer<28) hCorr->GetXaxis()->SetRange(100,250);
        hCorr->GetYaxis()->SetLabelSize(0.08);
        hCorr->GetXaxis()->SetLabelSize(0.08);
        hCorr->Draw("HIST");
        f->Draw("SAME");
        _gain_ssaCut = pk.gain;

        C_FFT_ssaCut[i_layer][i_chip]->cd(i_channel + 1);
        hFFT->Draw();
      ...
      C_FFT_ssaCut[i_layer][i_chip]->Write();
