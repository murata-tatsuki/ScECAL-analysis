...
{ 
  ...
  */
  ...
  fileout.mkdir("chip_canvas_quantized");
  // gain_histo->Write();
  ...
  for (int i_layer = 0; i_layer < layerNu; ++i_layer) {
    for (int i_chip = 0; i_chip < chipNu; ++i_chip) {
      for (int i_channel = 0; i_channel < channelNu; ++i_channel) {
        ...
        sprintf(char_tmp, "quantized_Layer%d_Chip%d_Chn%d", i_layer, i_chip, i_channel);
        h_MIP_quantize[i_layer][i_chip][i_channel] = new TH1D(char_tmp, char_tmp, bin_q, min_q, max_q);
        h_MIP_quantize[i_layer][i_chip][i_channel]->GetXaxis()->SetTitle("ADC");
        h_MIP_quantize[i_layer][i_chip][i_channel]->SetLineWidth(2);
        h_MIP_quantize[i_layer][i_chip][i_channel]->SetLineColor(4);
  ...
  fileout.cd();
  fileout.cd("chip_canvas_quantized");
  cout << "chip_canvas_quantized" << endl;
  for (int i_layer = 0; i_layer < layerNu; ++i_layer) {
    cout << i_layer << "/" << layerNu << endl;
    for (int i_chip = 0; i_chip < chipNu; ++i_chip) {
      for (int i_channel = 0; i_channel < channelNu; ++i_channel) {
        h_MIP_quantize[i_layer][i_chip][i_channel]->Write();
      }
    }
  }
  fileout.cd();
